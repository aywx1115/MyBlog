...
public class MyBlogController {
    ...
    public String search(HttpServletRequest request, @PathVariable("keyword") String keyword, @PathVariable("page") Integer page) {
        ...
        request.setAttribute("blogPageResult", blogPageResult);
        request.setAttribute("pageName", "Search Name");
    ...
    public Result comment(HttpServletRequest request, HttpSession session,
        ...
        if (StringUtils.isEmpty(verifyCode)) {
            return ResultGenerator.genFailResult("Code cannot be empty");
        }
        String kaptchaCode = session.getAttribute("verifyCode") + "";
        if (StringUtils.isEmpty(kaptchaCode)) {
            return ResultGenerator.genFailResult("Illegal request");
        }
        if (!verifyCode.equals(kaptchaCode)) {
            return ResultGenerator.genFailResult("Code error");
        }
        ...
        if (StringUtils.isEmpty(commentator)) {
            return ResultGenerator.genFailResult("Pls input name");
        }
        if (StringUtils.isEmpty(email)) {
            return ResultGenerator.genFailResult("Pls input Email");
        }
        if (!PatternUtil.isEmail(email)) {
            return ResultGenerator.genFailResult("Pls input correct Email format");
        }
        if (StringUtils.isEmpty(commentBody)) {
            return ResultGenerator.genFailResult("Pls input comment");
        }
        if (commentBody.trim().length() > 200) {
            return ResultGenerator.genFailResult("Comment is too long");
        }
     ...
     */
    @GetMapping({"/{subUrl1}"})
    public String detail(HttpServletRequest request, @PathVariable("subUrl1") String subUrl) {
