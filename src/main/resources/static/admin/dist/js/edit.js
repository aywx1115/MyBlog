            ...
            if (items && items.length > 0) {
                for (var item of items) {
                    if (item.type.startsWith("image/")) {
                        ...
                        if (!file) {
                            alert("Pls upload correct file format");
                            return;
                        }
    ...
    new AjaxUpload('#uploadCoverImage', {
        ...
        onSubmit: function (file, extension) {
            if (!(extension && /^(jpg|jpeg|png|gif)$/.test(extension.toLowerCase()))) {
                alert('Only support jpg、png、gif files！');
                return false;
            }
        },
...
$('#confirmButton').click(function () {
    ...
    if (!validLength(blogTags, 150)) {
        swal("Tags is too long", {
            icon: "error",
        });
        return;
    }
    if (isNull(blogContent)) {
        swal("Pls input connect", {
            icon: "error",
        });
        return;
    }
        ...
        success: function (result) {
            if (result.resultCode == 200) {
                ...
                swal({
                    ...
                    confirmButtonText: 'Return blog list',
